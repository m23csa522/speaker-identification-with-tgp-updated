...
class TemporalGatePooling(nn.Module):
    def __init__(self, config: Config):
        ...
        self.timewise_linear = nn.Linear(self.filter_size // self.num_head, self.filter_size // self.num_head)
    ...
    def forward(self, hidden_states: torch.Tensor, input_lengths: torch.Tensor = None) -> torch.Tensor:
        """
        Args:
        ...
        filters = self.timewise_linear(filters)
        # filters: [B, H, T, D]
        ...
        #out = torch.sum(values * gates, dim=-2)
        out = torch.sum(values * gates, dim=-2)  # [B, H, D]
        out = out.reshape(out.size(0), -1)       # [B, H * D]

        print("🧪 TemporalGatePooling output shape:", out.shape)
