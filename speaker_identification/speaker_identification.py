...
class SpeakerIdentificationModel(nn.Module):
    def __init__(self, config: dict):
        ...
        # Pass each config section separately
        self.wav_to_mel = WavToMel(config)
        self.encoder = ConformerModel(config["model"])
        self.pooling_layer = PoolingLayer(config["model"])
        self.fc_layer = FCLayer(config["model"])
